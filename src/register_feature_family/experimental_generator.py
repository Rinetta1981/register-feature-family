...
def _familiarization_forms(
    *,
    ...
    if speech_act == SpeechAct.REQUEST:
        request_content = EXPERIMENTAL_REQUEST_CONTENT[content_id]

        less_formal = request_content.verb.less_formal
        more_formal = request_content.verb.more_formal
        fixed = request_content.object_form
            ...
            for variant_code, target_text in mappings:
                records.append(
                    LexicalFamiliarizationRecord(
                        ...
                        context_text=(
                            f"{LEXICAL_FAMILIARIZATION_CODE} "
                            f"{content_code} "
                            f"{variant_code.value}"
                        ),
                        target_text=target_text,
                    )
                )
